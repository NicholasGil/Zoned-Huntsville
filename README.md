...
| `/api/webhooks/stripe` | Verifies the Stripe signature, dedupes on `event.id`, inserts an entitlements row, then sends a magic link. After the 2xx, may also send a plain purchase receipt. |
